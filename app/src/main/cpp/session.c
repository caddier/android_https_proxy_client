...
void *handle_events(void *a) {
    ...
    log_android(ANDROID_LOG_WARN, "Start events tunfd=%d thread %x", args->tun, thread_id);
        ...
        if (ready == 0)
            ...
            for (int i = 0; i < ready; i++) {
                if (ev[i].data.ptr == &ev_pipe) {
                    ...
                    // Check upstream, tun is upstream
                    ...
                    while (!error && is_readable(args->tun)) {
                        log_android(ANDROID_LOG_DEBUG, "====check_tun =====");
                        if (check_tun(args, &ev[i], epoll_fd, sessions, maxsessions) < 0) {
                            error = 1;
                            log_android(ANDROID_LOG_ERROR, "check tun failed ");
                        }
                    ...
                    if (session->protocol == IPPROTO_ICMP ||
                        session->protocol == IPPROTO_ICMPV6) {
                            log_android(ANDROID_LOG_DEBUG, "====check_icmp_socket =====");
                            check_icmp_socket(args, &ev[i]);
                            log_android(ANDROID_LOG_DEBUG, "====check_icmp_socket over=====");
                        }
                    else if (session->protocol == IPPROTO_UDP) {
                        log_android(ANDROID_LOG_DEBUG, "====check udp socket====");
                        while (!(ev[i].events & EPOLLERR) && (ev[i].events & EPOLLIN) &&
                               is_readable(session->socket))
                            check_udp_socket(args, &ev[i]);
                            log_android(ANDROID_LOG_DEBUG, "====check udp socket over====");
                    } else if (session->protocol == IPPROTO_TCP) {
                        log_android(ANDROID_LOG_DEBUG, "====check_tcp_socket====");
                        check_tcp_socket(args, &ev[i], epoll_fd);
                        log_android(ANDROID_LOG_DEBUG, "====check_tcp_socket end====");
                    }
