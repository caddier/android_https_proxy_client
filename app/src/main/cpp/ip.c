...
int check_tun(const struct arguments *args,
    ...
    if (ev->events & EPOLLERR) {
        log_android(ANDROID_LOG_ERROR, "read tun packet error");
        return -1;
    }
    ...
    if (ev->events & EPOLLIN) {
        ...
        if (length < 0) {
            ...
            if (length > max_tun_msg) {
                max_tun_msg = length;
                log_android(ANDROID_LOG_ERROR, "Maximum tun msg length %d", max_tun_msg);
            }

            // Handle IP from tun
            log_android(ANDROID_LOG_DEBUG, "======begin handle_ip======");
            handle_ip(args, buffer, (size_t) length, epoll_fd, sessions, maxsessions);
            log_android(ANDROID_LOG_DEBUG, "======end handle_ip======");
...
void handle_ip(const struct arguments *args,
    ...
    if (version == 4) {
        log_android(ANDROID_LOG_DEBUG, "======handle ipv4 ====");
        ...
        log_android(ANDROID_LOG_DEBUG, "======handle ipv6 ====");
    ...
    if (protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6) {
        ...
        log_android(ANDROID_LOG_WARN, "ICMP packet src port %u dst port %u!!!!", sport, dport);

    } else if (protocol == IPPROTO_UDP) {
        if (length - (payload - pkt) < sizeof(struct udphdr)) {
            log_android(ANDROID_LOG_ERROR, "UDP packet too short");
            return;
        }
        ...
        log_android(ANDROID_LOG_DEBUG, "##udp packet src port: %u dst port %u", sport,dport);
        // TODO checksum (IPv6)
    } else if (protocol == IPPROTO_TCP) {
        if (length - (payload - pkt) < sizeof(struct tcphdr)) {
            log_android(ANDROID_LOG_ERROR, "TCP packet too short");
            return;
        }
        ...
        log_android(ANDROID_LOG_DEBUG, "tcp packet src port %u dst port %u", sport, dport);
        ...
        log_android(ANDROID_LOG_ERROR, "Unknown protocol %d", protocol);
    }
    ...
    if (protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6) {
        log_android(ANDROID_LOG_DEBUG, "===handle icmp ====");
        handle_icmp(args, pkt, length, payload, uid, epoll_fd);
        log_android(ANDROID_LOG_DEBUG, "===handle icmp over====");
    }
    else if (protocol == IPPROTO_UDP) {
        log_android(ANDROID_LOG_DEBUG, "===handle udp ====");
        handle_udp(args, pkt, length, payload, uid, epoll_fd);
        log_android(ANDROID_LOG_DEBUG, "===handle udp over====");
    }

    else if (protocol == IPPROTO_TCP) {
        log_android(ANDROID_LOG_DEBUG, "===handle tcp ====");
        handle_tcp(args, pkt, length, payload, uid, epoll_fd);
        log_android(ANDROID_LOG_DEBUG, "===handle tcp over====");
    }
}
