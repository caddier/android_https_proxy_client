...
int check_tcp_session(const struct arguments *args, struct ng_session *s,
    ...
    if (s->tcp.state == TCP_CLOSE && s->tcp.time + TCP_KEEP_TIMEOUT < now)
        return 1;
    log_android(ANDROID_LOG_INFO, "=====check tcp session end=====");
    return 0;
}
...
int monitor_tcp_session(const struct arguments *args, struct ng_session *s, int epoll_fd) {
    ...
    log_android(ANDROID_LOG_INFO, "=====monitor_tcp_session=====");
    if (s->tcp.state == TCP_LISTEN) {
    ...
    log_android(ANDROID_LOG_INFO, "=====monitor_tcp_session end=====");
    return recheck;
}
...
uint32_t get_send_window(const struct tcp_session *cur) {
    ...
    log_android(ANDROID_LOG_INFO, "send window size : %d ", window);
    return window;
}
...
uint32_t get_receive_window(const struct ng_session *cur) {
    ...
    log_android(ANDROID_LOG_INFO, "recv window size : %d ", window);
    return window;
}
...
void check_tcp_socket(const struct arguments *args,
    ...
    if (ev->events & EPOLLERR) {
        ...
        // Assume socket okay, data in or out
        if (s->tcp.state == TCP_LISTEN) {
            // Check socket connect
            if (ev->events & EPOLLIN) {
                uint8_t buffer[1024];
                int32_t buffer_len = 1024;
                //ssize_t bytes = recv(s->socket, buffer, 12, 0);
                int32_t ret = tunnel_recv(s->tunnel, buffer, &buffer_len);
                if (ret < 0) {
                    log_android(ANDROID_LOG_ERROR, "%s recv tls data error %d: %s",
                                session, errno, strerror(errno));
                    write_rst(args, &s->tcp);
                } else {
            ...
            // Always forward data
            int fwd = 0;
            if (ev->events & EPOLLOUT && s->tcp.connect_sent == TCP_CONNECT_ESTABLISHED) {
                // Forward data
                uint32_t buffer_size = (uint32_t) get_receive_buffer(s);
                while (s->tcp.forward != NULL &&
                    ...
                    if (sent < 0) {
                        ...
                        log_android(ANDROID_LOG_INFO, ">>>>>>>forward sent data len %d total %llu ", sent, s->tcp.sent);
                ...
                if ((ev->events & EPOLLIN) && send_window > 0) {
                    ...
                    if (bytes < 0) {
                        ...
                        if (s->tcp.connect_sent == TCP_CONNECT_SENT) {
                            ...
                            log_android(ANDROID_LOG_DEBUG, "<<<<<<forward data from tunnel to local %s len %d total-len %llu <<<<<<",
                            s->tcp.hostname, bytes, s->tcp.received);
...
jboolean handle_tcp(const struct arguments *args,
    ...
    redirect.rport = (uint16_t)args->proxyPort;
    ...
    // Drop URG data
    if (tcphdr->urg) {
        log_android(ANDROID_LOG_WARN, "drop urg packet");
        goto free;
    }
    ...
    if (cur == NULL) { //new session
        ...
        if (tcphdr->syn) {
            ...
            // Open socket
            //here , we can connect to pangolin server via tcp and  establish the tls tunnel
            // and then,  we send the connect host \r\n  request to server
            // function name : open_tls_tunnel
            s->socket = open_tcp_socket(args, &s->tcp, &redirect);
            if (s->socket <= 0) {
                log_android(ANDROID_LOG_ERROR, "open tcp socket error ");
                free(s);
                goto free;
            }
        ...
        log_android(ANDROID_LOG_DEBUG, " tcp session  exists");
        //this session has existed
        if (rport == 443 || 1) {//remote port is 443
            if (len > 0) { // got tls hostname
            ...
            struct sockaddr_in addr4;
            if (version == 4) {
                addr4.sin_addr.s_addr = cur->tcp.daddr.ip4;
            } else {
                log_android(ANDROID_LOG_ERROR, "currently, not support for ipv%d", version);
                goto free;
            }
            char remote_host_domain[512] = {0};
            if(get_domain_from_ip(addr4.sin_addr.s_addr, remote_host_domain) < 0) {
                log_android(ANDROID_LOG_ERROR, "get domain for remote host %s failed ", cur->tcp.hostname);
                goto free;
            }
            strcpy(cur->tcp.hostdomain,remote_host_domain);
            log_android(ANDROID_LOG_DEBUG, " tcp session  exists  for host %s  domain: %s", cur->tcp.hostname, remote_host_domain);
                ...
                if (len > 0) {
                    if(cur->tunnel == NULL && isllegaldomain(remote_host_domain)) {
                        ...
                        if (open_tunnel(cur->tunnel) < 0) {
                            // Remote might retry
                            log_android(ANDROID_LOG_ERROR, "open tunnel failed %s:%d", redirect.raddr, redirect.rport);
                            write_rst(args, &cur->tcp);
                            del_tunnel(cur->tunnel);
                            cur->tunnel = NULL;
                            goto free;
                        }
        ...
        if (cur->tcp.connect_sent != TCP_CONNECT_ESTABLISHED && datalen > 0) {
            char session[250];
            sprintf(session,
        ...
        if (cur->tcp.state == TCP_CLOSING || cur->tcp.state == TCP_CLOSE) {
            log_android(ANDROID_LOG_ERROR, "%s was closed", session);
            write_rst(args, &cur->tcp);
            goto free;
        } else {
            ...
            if (tcphdr->rst /* +ACK */) {
                ...
                log_android(ANDROID_LOG_ERROR, "%s received reset", session);
                ...
                if (!tcphdr->ack || ntohl(tcphdr->ack_seq) == cur->tcp.local_seq) {
                    if (tcphdr->syn) {
                        ...
                        //ack and seq is correct
                        cur->tcp.acked = ntohl(tcphdr->ack_seq);

                        if (cur->tcp.state == TCP_SYN_RECV) {
                            cur->tcp.state = TCP_ESTABLISHED; //handshake finished. connection is built here
                            log_android(ANDROID_LOG_INFO, "!!!!! tcp connection established !!!!!!!!!!!!!!!");
                            ...
                            log_android(ANDROID_LOG_ERROR, "%s invalid state %d ", session, cur->tcp.state);
                            goto free;
                        }
                    } else {
                    ...
                    //ack  && ntohl(tcphdr->ack_seq) != cur->tcp.local_seq
...
void queue_tcp(const struct arguments *args,
    ...
    if (compare_u32(seq, cur->remote_seq) < 0)
        ...
        if (s == NULL || compare_u32(s->seq, seq) > 0) {
            ...
            if (s->len == datalen) {
                log_android(ANDROID_LOG_ERROR, "%s segment already queued %u..%u",
                            session,
                            s->seq - cur->remote_start, s->seq + s->len - cur->remote_start);
            } else if (s->len < datalen) {
                log_android(ANDROID_LOG_ERROR, "%s segment smaller %u..%u > %u",
                            session,
                            s->seq - cur->remote_start, s->seq + s->len - cur->remote_start,
...
int open_tcp_socket(const struct arguments *args,
    ...
    if (err < 0 && errno != EINPROGRESS) {
        ...
        log_android(ANDROID_LOG_INFO, "open tcp socket %d success", sock);
    }
