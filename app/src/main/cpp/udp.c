...
int check_udp_session(const struct arguments *args, struct ng_session *s,
    ...
    log_android(ANDROID_LOG_INFO, "====check udp ======");
...
jboolean handle_udp(const struct arguments *args,
    ...
    if (cur != NULL && cur->udp.state != UDP_ACTIVE) {
        log_android(ANDROID_LOG_WARN, "UDP ignore session from %s/%u to %s/%u state %d",
    ...
    if (ntohs(udphdr->dest) == 53) {
        log_android(ANDROID_LOG_DEBUG, "DNS Query request!!!!!!!");
        ...
        if (get_dns_query(args, &cur->udp, data, datalen, &qtype, &qclass, qname) >= 0) {
            ...
            if (check_domain(args, &cur->udp, data, datalen, qclass, qtype, qname)) {
                ...
                int32_t response_len = 2048;

                if (create_dns_response(data,datalen,response, &response_len) < 0) {
                    log_android(ANDROID_LOG_ERROR, "create dns response failed");
                } else {
                    //parse_dns_response(args, &cur->udp, response, &response_len);
                    write_udp(args, &cur->udp, response, response_len);
                }
            ...
            log_android(ANDROID_LOG_ERROR, "get dns query failed ");
        }
    }
