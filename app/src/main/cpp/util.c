...
char *hex(const u_int8_t *data, const size_t len, char **hex_str_out) {
...
void hex_free(char * hex) {
    if( hex != NULL)
        free(hex);
}

void dump_buffer(char *buffer, int32_t len) {
    char *data_hex_str = NULL;
    hex(buffer, len, &data_hex_str);
    log_android(ANDROID_LOG_INFO, "data: %s ", data_hex_str);
    hex_free(data_hex_str);
}
